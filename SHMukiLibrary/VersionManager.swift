...
class VersionManager: NSObject {
    ...
    func isUpdateAvailable(completion: @escaping (Bool?, Error?) -> Void) throws -> URLSessionDataTask {
        guard let info = Bundle.main.infoDictionary,
            ...
            let url = URL(string: "http://itunes.apple.com/lookup?id=" + apple_id) else {
        ...
        let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
            do {
                ...
                /*
                let _bundleVersion = currentVersion
                let _appstoreVersion = version
                print(result["version"] as? String)
                print("_bundleVersion = \(_bundleVersion), _appstoreVersion = \(_appstoreVersion)")
                if _appstoreVersion > _bundleVersion {
                    completion(true, nil)
                }else{
                    completion(false, nil)
                }
