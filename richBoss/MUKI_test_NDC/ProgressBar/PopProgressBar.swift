...
//
/*当然这个项目是最近为自定义弹窗ZHFToolBox项目添加的很小一部分
 ZHFToolBox地址链接:https://github.com/FighterLightning/ZHFToolBox.git
...
class PopProgressBar: UIView ,UIGestureRecognizerDelegate {
    ...
    func addProgressView() {
        ...
        self._bgView.frame.size = CGSize(width: self.frame.width*0.9, height: 250)
        ...
        self.title = UILabel()
        title.frame.size = CGSize(width: self._bgView.frame.width*0.8, height: 30)
        title.frame.origin = CGPoint(x: self._bgView.frame.minX - self.frame.width*0.05 + 25 , y: 10)
        title.font = UIFont.boldSystemFont(ofSize: 18)
        title.text = "檔案下載中，請稍候"
        self._bgView.addSubview(title)
        
        self._filesTitle = UILabel()
        _filesTitle.frame.size = CGSize(width: self._bgView.frame.width*0.8, height: 30)
        _filesTitle.frame.origin = CGPoint(x: self._bgView.frame.minX - self.frame.width*0.05 + 25 , y: 60)
...
//        self._filesValue = _files.count
        ...
        _downloadTitle.frame.origin = CGPoint(x: self._bgView.frame.minX - self.frame.width*0.05 + 25 , y: 110)
        _downloadTitle.font = UIFont.systemFont(ofSize: 14)
        //_downloadTitle.text = "正在下載檔案"
        self._bgView.addSubview(_downloadTitle)
        
        progressView = UIView.init(frame: CGRect.init(x: self._bgView.frame.minX - self.frame.width*0.05 + 25 , y: 180, width: self._bgView.frame.width-50, height: 15))
        progressView.layer.masksToBounds = true
        progressView.layer.cornerRadius = 7.5
        progressView.backgroundColor = UIColor.gray
        self._bgView.addSubview(progressView)
        ...
        self._bgView.addSubview(hintBtn)
    ...
    @objc func gradentWith(frame:CGRect) {
        ...
        pathAnimation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName(string: "linear") as String)
